...
function calcEquiSol( i, year ) {
	...
	//document.write(i + ' - ' + UTC.toString() + '<br>');
	document.write(i + ' - ' + formatedDate(UTC) + '<br>');
	console.log(UTC);
...
function formatedDate(data){
	var returnstring = "";
	returnstring += data.getFullYear()+'-';
	returnstring += (data.getMonth() + 1)+'-';
	returnstring += data.getDate()+' ';

	returnstring += data.getHours() + ':';
	returnstring += (data.getMinutes() < 10 ? '0' + data.getMinutes() : data.getMinutes()) + ':';
	returnstring += data.getSeconds();
	return returnstring;
};
